...
k_means_clustering()
{
    ...
    while(delta / (double)N > THRESHOLD)
	{
        ...
        if(PID == 0)
        {
            ...
            for(i = 1; i < p_size; i++)
            {
                MPI_Send(&objects[offset], chunk, MPI_DOUBLE, i, TAG, MPI_COMM_WORLS);
                offset += chunk;
            }
        ...
        {
            offset = N - (p_size -1) * chunk;
            MPI_Recv(&objects[offset + (PID - 1) * chunk], chunk, MPI_DOUBLE, 0, TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        ...
        if(PID == 0)
        {
            ...
            for(i = 1; i < p_size; i++)
            {
                MPI_Recv(&membership[offset], chunk, MPI_INT, i, TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                double tmp_delta = 0;
                MPI_Recv(&tmp_delta, 1, MPI_DOUBLE, i, 5, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                delta += tmp_delta;
                offset += chunk;
            }
        }
        ...
        {
            offset = N - (p_size - 1) * chunk + (PID - 1) * chunk;
            MPI_Send(&membership[offset], chunk, MPI_INT, 0, TAG, MPI_COMM_WORLD);
        ...
        {
            for(i = 1; i < p_size; i++)
                MPI_Send(&delta, 1, MPI_DOUBLE, i, 6, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        else
        {
            MPI_Recv(&delta, 1, MPI_DOUBLE, i, 6, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
